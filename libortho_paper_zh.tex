...
\begin{proof}[证明思路]
考虑影响函数（Influence Function）~\cite{influence-functions}，它估计如果样本$z_i$被小幅加权$\delta$时参数的变化$\Delta \theta$：
\[
\Delta \theta \approx -H^{-1} \nabla \ell(z_i, \theta^*)
\]
如果$z_i$代表通用知识，其梯度$\nabla \ell(z_i)$应该与数据集协方差的主方向对齐，在良好泛化的模型中，这通常与Hessian的平坦方向对齐。相反，记忆化的样本（异常值）产生的梯度与通用共识冲突，推动权重向高曲率方向移动，以最小化$z_i$的特定损失而不破坏全局最小值。因此，对于记忆化点，梯度能量集中在高特征值（高曲率）子空间$\mathcal{S}_{mem}$中。
\end{proof}
...
该过程确保高曲率方向（可能包含记忆化信息）被识别并分离到正交流中。
